...
def update_single_unlisted_stock_price_by_month(stock_no: str, date: str, db: TWSEdb = None):
    count = 0
    if TWSEdb is None:
        db = TWSEdb("stock_twse", "basePrice")
    ...
    if res != "":
        ...
        for d in data:
            ...
            start_p = detail[3].text
            end_p = detail[6].text
            amount = detail[1].text.replace(",", "") * 1000

            db.add_price_data(int(stock_no), new_date, float(high_p), float(low_p), float(start_p), float(end_p), amount)
...
def update_all_unlisted_stock_price_by_month(date: str):
    ...
    for key in unlisted_dict:
        if len(key) < 5:
            try:
                time.sleep(update_interval)
                update_single_unlisted_stock_price_by_month(str(key), date, db)
            except Exception as e:
                print(e)
